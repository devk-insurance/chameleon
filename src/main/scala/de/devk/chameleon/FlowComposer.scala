...
class FlowComposer(config: Config, hostTagService: HostTagService, jmxManager: JmxManager)(implicit system: ActorSystem) extends Logging {

  private val tcpServerService = new TcpServerService(config, hostTagService, jmxManager)
